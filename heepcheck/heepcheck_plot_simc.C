...
{ 
  ...
  //*****Beam energy, e-angle, e-momentum, p-angle, p-momentum, beam_offset, e-angle offset, e-momentum affset, p-angle offset, p-momentum offset
    ifstream myfile;
    myfile.open("heepcheck_simc.input");
    while (myfile.eof() == false) {
     myfile >> En >> theta_ee >> p_Ecen >> theta_pp >> p_Pcen >> E_off >> theta_Eoff >> p_eoff >> theta_poff >> p_poff;
    }
    myfile.close();
  ...
  TChain * chain = new TChain(Form("h%d",666));
  chain->Add("eep_optics.root");
  // cout <<run<<endl;
